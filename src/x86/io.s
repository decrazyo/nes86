...
IO_FUNC_INDEX_SIZE = rbaIoFuncIndexEnd - rbaIoFuncIndex
...
.proc get_handler_index
    ldx #0 ; io_error index

    ; check if the I/O port is outside of our table
    lda Reg::zwS0X+1
    cmp #>IO_FUNC_INDEX_SIZE
    bne skip_low_cmp
    lda Reg::zwS0X
    cmp #<IO_FUNC_INDEX_SIZE
skip_low_cmp:
    ...
    ; create a pointer to our I/O port function index table
    lda #<rbaIoFuncIndex
    adc Reg::zwS0X
    sta Tmp::zw1
    lda #>rbaIoFuncIndex
...
out_of_range:
    ; the caller can now use X to index function tables
    rts
.endproc
